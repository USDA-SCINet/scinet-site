...
* For larger (e.g., greater than 1 GB) data transfers:
  * [Globus Connect Personal](#globus-connect-personal), an application on your local computer that allows you to transfer files via Globus, which provides a web-based interface.
<!--excerpt-->
...
**Please note:** Using Open OnDemand for file transfer is only for transfers between your local computer and Ceres or Atlas. Transfers to and from Juno are not supported because Open OnDemand is intended for compute clusters and is not available on Juno. 
...
`scp` is usually available on any Linux or MacOS computer and on Microsoft Windows 10 or greater (in PowerShell). 
...
```
To learn more about the `scp` command and all available options, run `man scp`.
