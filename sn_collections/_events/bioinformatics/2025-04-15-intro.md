...
- Quality scores are encoded as ASCII characters. The rule for converting an ASCII character to an integer varies
...
`bcftools` is a command-line tool used for viewing, filtering and manipulating VCF files. We will use `bcftools` to help us summarize variant info and for extracting and selecting variant. 
...
{% include alert no_icon=true type="success" title="You try:" text="<ul><li>How many high quality (>=30) SNPs are on Chr5?</li><li>How many SNPs pass the quality threshold?</li></ul>" %}
